...
trait OHLC
{
    ...
    public function markOHLC($ticker, $bf = false, $bf_pair = 'BTC/USD')
    {
        ...
        $last1m = \DB::table('btrade_ohlc_1m')->select(DB::raw('MAX(timeid) AS timeid'))
        ...
        if ($last1timeid < $timeid) {
            ...
            $accum1ma = \DB::table('btrade_ohlc_tick')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum1mb = \DB::table('btrade_ohlc_tick')->select(DB::raw('open AS open'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last1timeids)
                ->where('timeid', '<=', ($last1timeids + 59))
                ->limit(1)
            ...
            $accum1mc = \DB::table('btrade_ohlc_tick')->select(DB::raw('close AS close'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last1timeids)
                ->where('timeid', '<=', ($last1timeids + 59))
                ->orderBy('ctime', 'desc')
        ...
        $last5m = \DB::table('btrade_ohlc_5m')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last5timeid < $timeid) {
            ...
            $accum5ma = \DB::table('btrade_ohlc_1m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum5mb = \DB::table('btrade_ohlc_1m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last5timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum5mc = \DB::table('btrade_ohlc_1m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last5timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
        ...
        $last15m = \DB::table('btrade_ohlc_15m')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last15timeid < $timeid) {
            ...
            $accum15ma = \DB::table('btrade_ohlc_5m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum15mb = \DB::table('btrade_ohlc_5m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last15timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum15mc = \DB::table('btrade_ohlc_5m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last15timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
        ...
        $last30m = \DB::table('btrade_ohlc_30m')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last30timeid < $timeid) {
            ...
            $accum30ma = \DB::table('btrade_ohlc_15m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum30mb = \DB::table('btrade_ohlc_15m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last30timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum30mc = \DB::table('btrade_ohlc_15m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last30timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
        ...
        $last60m = \DB::table('btrade_ohlc_1h')->select(DB::raw('MAX(timeid) AS timeid'))
            ->where('instrument', $bf_pair)
            ->get();
        ...
        if ($last60timeid < $timeid) {
            ...
            $accum60ma = \DB::table('btrade_ohlc_30m')->select(DB::raw('MAX(high) as high, MIN(low) as low'))
                ->where('instrument', $bf_pair)
            ...
            $accum60mb = \DB::table('btrade_ohlc_30m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last60timeids)
                ->where('timeid', '<=', ($timeid))
                ->limit(1)
            ...
            $accum60mc = \DB::table('btrade_ohlc_30m')->select(DB::raw('*'))
                ->where('instrument', $bf_pair)
                ->where('timeid', '>=', $last60timeids)
                ->where('timeid', '<=', ($timeid))
                ->orderBy('ctime', 'desc')
    ...
    public function getRecentData($pair='BTC/USD', $limit=168, $day_data=false, $hour=12, $periodSize='1m', $returnRS=false)
    {
        ...
        $a = \DB::table('btrade_ohlc_'.$periodSize)
