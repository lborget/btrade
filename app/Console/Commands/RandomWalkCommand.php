...
use btrade\Traits\CandleMap;
use btrade\Traits\OHLC;
use btrade\Traits\Pivots;
use btrade\Traits\Signals;
use btrade\Traits\Strategies;
...
class RandomWalkCommand extends Command {
     ...
     * @var string
     */
    protected $name = 'btrade:random_walk';
    ...
    public function updateDb()
    {
        ...
        $ids = \DB::table('btrade_strategy')->select(DB::raw('unix_timestamp(ctime) AS stime, position_id, pair'))->whereNull('profit')->get();;
        if (!empty($ids)) {
            foreach ($ids as $id) {
                $modify = 301;
                if ($id->stime + $modify < time()) {
                    ...
                    if ($err) {
                        print_r($order);
                        #\DB::table('btrade_strategy')->where('position_id', $id->position_id)->update(['profit' => '0', 'state' => 'error', 'close_reason' => 'error']);
                        return;
                    }
                    $prof = $order['profit'] ?? null;
                    if (!is_null($prof)) {
                        \DB::table('btrade_strategy')->where('position_id', $id->position_id)->update(['profit' => $prof, 'state' => $order['state'], 'close_reason' => $order['close_reason']]);
                    }
                }
            }
        }
    ...
    public function createPosition($instrument, $direction, $strategy, $pos=0, $neg=0, $size=2, $lev=200)
    {
        ...
        $insert['state']       = 'active';
        \DB::table('btrade_strategy')->insert($insert);
    ...
    public function handle()
    {
        ...
        foreach($strategies as $k => $strategy) {
            $strategies[$k] = str_replace('btrade_','',$strategy);
        }
